...
src/data_analysis_agent/capabilities/sampling/slicing.py = "缓存结果查询下取:单谓词 filter + 列投影 + head/tail/sample 模式(D6)"
src/data_analysis_agent/capabilities/sampling/json_digest.py = "JSON/JSONL 结构摘要:键路径骨架 + 数组长度分布 + 代表元素采样(D7)"
...
src/data_analysis_agent/sampling/slicing.py = "v1 shim → capabilities/sampling/slicing(D6 查询下取)"
src/data_analysis_agent/sampling/json_digest.py = "v1 shim → capabilities/sampling/json_digest(D7 JSON 摘要)"
