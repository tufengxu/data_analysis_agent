...
class ResultStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._index_stamp: tuple[int, int] | None = None  # (mtime_ns, size) last seen
        try:
    ...
    def _index_changed_on_disk(self) -> bool:
        """True when the on-disk index was rewritten by another process.

        v2: two capability-server processes may share one store directory
        (base MCP-client instance + adapter plugin instance). Index rewrites
        are atomic full rewrites, so mtime+size stamps detect them cheaply.
        """
        try:
            stat = self.index_path.stat()
        except OSError:
            return self._index_stamp is not None
        stamp = (stat.st_mtime_ns, stat.st_size)
        return self._index_stamp is not None and stamp != self._index_stamp
    ...
    def _load_index(self) -> None:
        ...
        if stat is not None:
            self._index_stamp = (stat.st_mtime_ns, stat.st_size)
    ...
    def put(self, result_id: str, content: str, meta: dict[str, Any]) -> bool:
        if not self._available:
            return False
        # v2 多进程共享:写入前先吸收其他进程的 index 增量,避免全量重写覆盖
        # 他人条目。stat→rewrite 之间仍有小竞争窗(best-effort,丢失仅退化为
        # 该条不可回取,不影响主流程)。
